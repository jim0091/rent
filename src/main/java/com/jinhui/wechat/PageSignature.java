...
public class PageSignature {
    ...
    public static void fetchSignature(final SignatureParameter signatureParameter)
            throws InterruptedException, ExecutionException, TimeoutException {
        CompositionHttpClient.httpsRequest(new CompositionHttpClient.Action() {
            @Override
            public void doAction(HttpClient httpClient)
                ...
                signatureParameter.setTicket(ticket);
                //
                genSignature(signatureParameter);
    ...
    private static void genSignatureAndWechatJs(final HttpServletRequest request, String ticket){
        ...
        if(!StringUtils.isEmpty(sharedKey)) {
            String[] title_desc = SharedInfo.getProperty(sharedKey).split("/");
            /*if(title_desc.length > 2) {
                String confessionId = relativeUrl.substring(relativeUrl.lastIndexOf("/")+1);
                RentService confessionService = SpringContextHolder.getBean("RentService");
                if(!confessionService.isDoingConfession(confessionId)) {
                    //想被告白
                    title_desc[0] = title_desc[2];
                    title_desc[1] = title_desc[3];
                }
            }*/
    ...
    private static void genSignature(SignatureParameter signatureParameter){
        String sig_src = SignatureString
                .replace(signatureParameter.ticket().key(), signatureParameter.ticket().value())
                .replace(signatureParameter.nonceStr().key(), signatureParameter.nonceStr().value())
                .replace(signatureParameter.timestamp().key(), signatureParameter.timestamp().value())
                .replace(signatureParameter.link().key(), signatureParameter.link().value());
        String signature = org.springframework.data.redis.core.script.DigestUtils.sha1DigestAsHex(sig_src);
        if(logger.isInfoEnabled()) {
            logger.info("signature src => {},\nsignature={}",sig_src, signature);
        }
        signatureParameter.setSignature(signature);
    }
        ...
        WechatDataCache.instance().put(WechatJsAbsPath, new WechatDataCache.FileValuesAction(jsFilename) {
            ...
            String doAction() {
                String sharedConfig = SharedConfig
                        ...
                        .replace(parameters.desc().key(), parameters.desc().value())
